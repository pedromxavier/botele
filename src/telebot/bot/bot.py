...
class TeleBot(metaclass=MetaTeleBot):
    ...
    def add_query_handlers(self):
        for callback, pattern in self.query_handlers:
            ...
            stdout[3] << f"> Add Query Handler: [{pattern}] @{callback}"

    def add_error_handler(self):
        if self.error_handler is not None:
            self.dispatcher.add_error_handler(self.error_handler)
            stdout[3] << f"> Add Error Handler: @{self.error_handler}"
    ...
    def keyboard_markup(cls, key:str, *rows) -> InlineKeyboardMarkup:
        ...
        for row in rows:
            ...
            for value in row:
                ...
                button = InlineKeyboardButton(text, callback_data=callback_data, **kwargs)
                stdout << f">>> Add key `{callback_data}`"
